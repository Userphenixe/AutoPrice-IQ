...
# .../AUTOPRICE-IQ/ETL/data/leboncoin.csv
DEFAULT_DEST_FILE = BASE_DIR / "data" / "leboncoin.csv"
...
def parse_params(raw: str | None):
    """Parse la ligne de paramètres : '2018 · 139500 km · Essence · Manuelle'."""
    if not raw:
        return "", "", "", ""
...
async def _async_scrape_leboncoin(nb_pages: int) -> list[dict]:
    """Fonction async centrale : ouvre le navigateur, scrape N pages et renvoie toutes les lignes."""
...
def run_leboncoin(nb_pages: int = NB_PAGES, dest_file: Path | str | None = None) -> Path:
    """
    Point d'entrée **synchrone** à utiliser dans Airflow ou en local.
