...
DEST_FILE = BASE_DIR / 'data' / 'aramisauto.csv'     # .../AUTOPRICE-IQ/ETL/data/aramisauto.csv
...
def parse_bottom(txt: str | None):
    ...
    cond = "Occasion" if "occasion" in txt.lower() else ("Neuf" if "neuf" in txt.lower() else "")
    return year, km, cond
...
def guess_fuel_details(txt: str | None):
    ...
    for key in ["électrique", "electrique", "essence", "diesel", "hybride", "micro-hybride", "gpl"]:
        if key in low:
            ...
            if fuel == "Electrique": fuel = "Électrique"
            break
    gearbox = "Automatique" if "auto" in low or "automatique" in low else ("Manuelle" if "manu" in low else "")
    return fuel, gearbox
